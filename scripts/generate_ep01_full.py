#!/usr/bin/env python3
"""Generate full EP01 audio using Chatterbox TTS with English reference voice."""
...
# EP01 full script with [pause] markers replaced by natural punctuation
...
print("Generating full EP01 (exaggeration=0.5)...")

start = time.time()
wav = model.generate(SCRIPT, audio_prompt_path=REF_AUDIO, exaggeration=EXAGGERATION, cfg_weight=0.5)
...
print(f"Gen time: {elapsed:.0f}s | RTF: {elapsed/duration:.1f}x")
